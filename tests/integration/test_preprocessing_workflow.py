...
class TestRunHydroProcessorEndToEnd:
    ...
    def test_run_hydro_processor_with_static_features(self, temp_dir, create_basin_files, basin_ids):
        ...
        # Should have static features output
        assert result.processed_static_attributes_path is not None
    ...
    def test_run_hydro_processor_quality_validation_failure(self, mock_clean_data, temp_dir, create_basin_files, basin_ids):
        """Test handling when all basins fail quality validation."""
        # Mock clean_data to return no valid basins
        mock_clean_data.return_value = (pl.DataFrame(), {
            basin_id: Mock(passed_quality_check=False, failure_reason="Mock failure")
...
class TestPreprocessingWorkflowIntegration:
    ...
    def test_pipeline_fitting_transformation_integration(self, synthetic_clean_data):
        ...
        # Fit pipelines
        fitted_pipelines = fit_time_series_pipelines(
            synthetic_clean_data,
            features_pipeline,
            target_pipeline
        )
        
        # Transform data
        transformed_data = transform_time_series_data(
            synthetic_clean_data,
            fitted_pipelines
        )
...
class TestEndToEndWorkflowScenarios:
    ...
    def test_complex_workflow_all_features(self, temp_dir, create_basin_files, basin_ids):
        ...
        # Verify splits exist for multiple basins
        train_files = list((result.processed_timeseries_dir / "train").glob("*.parquet"))
        val_files = list((result.processed_timeseries_dir / "val").glob("*.parquet"))
        test_files = list((result.processed_timeseries_dir / "test").glob("*.parquet"))
