...
def synthetic_clean_data(basin_ids, date_range):
    ...
    for basin_id in basin_ids:
        ...
        streamflow = np.maximum(
            0.1, streamflow_base + streamflow_temp_effect + np.random.normal(0, 5, n_days) + basin_offset * 2
        )
...
def preprocessing_config_missing_pipeline():
    ...
    return {"features": {"strategy": "unified", "columns": ["temperature", "precipitation"]}}


@pytest.fixture
def region_dirs(temp_dir):
    """Create temporary directory structure for region data."""
    regions = {"basin": temp_dir / "time_series" / "basin", "river": temp_dir / "time_series" / "river"}

    static_regions = {"basin": temp_dir / "static" / "basin", "river": temp_dir / "static" / "river"}
    ...
    return {"time_series": regions, "static": static_regions}
...
@pytest.fixture(params=["clean_data", "data_with_gaps", "data_with_outliers"])
