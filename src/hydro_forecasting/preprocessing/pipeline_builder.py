...
class PipelineSection:
    ...
    def strategy(self, strategy_name: str, **kwargs) -> PipelineSection:
        ...
        if strategy_name not in valid_strategies:
            raise ValueError(
                f"{self.section_name}: Invalid strategy '{strategy_name}'. Valid strategies: {valid_strategies}"
            )
    ...
    def is_complete(self) -> bool:
        ...
        return self._transforms is not None and self._strategy is not None and self._columns is not None
...
class PipelineBuilder:
    ...
    def build(self) -> dict[str, dict[str, Any]]:
        ...
        if not self._sections:
            raise ValueError(
                "No sections configured. Use features(), target(), or static_features() to configure sections."
            )
    ...
    def _build_section_config(self, section: PipelineSection) -> dict[str, Any]:
        ...
        sklearn_pipeline = create_sklearn_pipeline(section_config["transforms"], columns=section_config["columns"])

        # Wrap in appropriate pipeline wrapper
        if section_config["strategy"] == "unified":
            wrapped_pipeline = UnifiedPipeline(pipeline=sklearn_pipeline, columns=section_config["columns"])
        else:  # per_group
