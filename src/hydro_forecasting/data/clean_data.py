...
def validate_basin_quality(
    lf: pl.LazyFrame,
    config,
    raise_on_failure: bool = True,
) -> tuple[pl.DataFrame, dict[str, BasinQualityReport]]:
    """
    Validate that each basin has sufficient data (min_train_years) for training.
    Does not perform data transformations - only validation.
    ...
    Args:
        ...
        config: Configuration object with required_columns, group_identifier,
               min_train_years, and train_prop.
    ...
    Returns:
        Tuple of (original_df, reports) containing the original DataFrame and quality reports.
    ...
    # Step 2: Collect to eager DataFrame for validation
    try:
        df = lf.collect()
    except Exception as e:
        raise DataProcessingError(f"Data collection failed: {e}") from e

    # Step 3: Build reports per basin and check data sufficiency
    ...
    for basin_id in basin_ids:
        ...
        # Valid period info (basic validation info)
        valid_period: dict[str, dict[str, str | None]] = {}
        for c in cols:
        ...
        # Assemble report (no imputation info since we're not doing transformations)
        report = BasinQualityReport(
            valid_period=valid_period,
            processing_steps=[
                "quality_validation_only",
            ],
            imputation_info={},  # No imputation performed in validation
        )
    ...
    # Filter DataFrame to only valid basins
    try:
        filtered_df = df.filter(pl.col(gid).is_in(valid_basin_ids)) if valid_basin_ids else pl.DataFrame()
    except Exception as e:
        raise DataProcessingError(f"Failed to filter DataFrame: {e}") from e
...
def apply_cleaning_steps(
    ...
    """
    Apply data cleaning transformations to the LazyFrame.
    ...
    try:
        ...
        if existing_helpers:
            df = df.drop(existing_helpers)
    except Exception as e:
        raise DataProcessingError(f"Data processing failed: {e}") from e

    logger.info("Applied cleaning steps to data")
...
def clean_data(
    ...
    This function combines both cleaning and validation steps with full imputation reporting.
    This is the legacy function that some tests still depend on.
...
def _apply_cleaning_steps_with_imputation_tracking(
    lf: pl.LazyFrame,
    config,
) -> pl.DataFrame:
    """
    Apply cleaning steps while keeping helper columns for imputation tracking.
    """
    cols = config.required_columns
    max_gap = config.max_imputation_gap_size
    gid = config.group_identifier
    ...
    try:
        ...
        # Step 6: Collect to eager DataFrame (keep helper columns for imputation tracking)
        df = lf.collect()
    except Exception as e:
        raise DataProcessingError(f"Data processing failed: {e}") from e
...
def _validate_and_build_reports_with_imputation(
    df: pl.DataFrame,
    config,
    raise_on_failure: bool = True,
) -> tuple[pl.DataFrame, dict[str, BasinQualityReport]]:
    """
    Validate data and build quality reports with imputation information.
    """
    cols = config.required_columns
    max_gap = config.max_imputation_gap_size
    gid = config.group_identifier
    ...
    # Build reports per basin and check data sufficiency
    reports: dict[str, BasinQualityReport] = {}
    valid_basin_ids: list[str] = []
    failed_basins: list[str] = []
    ...
    # Filter DataFrame to only valid basins and drop helper columns
    try:
        filtered_df = df.filter(pl.col(gid).is_in(valid_basin_ids)) if valid_basin_ids else pl.DataFrame()
